...
public class UserFileController {
    ...
    public ResponseEntity<InputStreamResource> downloadFile(
        ...
        var fileContentOpt =
                fileStorageService.retrieveFile(fileId, currentTenantId).join();
        if (fileContentOpt.isEmpty()) {
            log.debug("File not found for user {} in tenant {} - fileId: {}", currentUserId, currentTenantId, fileId);
        ...
        if (!metadata.tenantId().equals(currentTenantId) || !metadata.authorId().equals(currentUserId)) {
            log.warn(
    ...
    public ResponseEntity<FileMetadataResponse> getFileMetadata(
        ...
        var metadataOpt =
                fileStorageService.getFileMetadata(fileId, currentTenantId).join();
        if (metadataOpt.isEmpty()) {
            log.debug(
                    "Metadata not found for user {} in tenant {} - fileId: {}", currentUserId, currentTenantId, fileId);
            return ResponseEntity.notFound().build();
        }
    ...
    public ResponseEntity<Void> deleteFile(
        ...
        Boolean deleted = fileStorageService.deleteFile(fileId, currentTenantId).join();
    ...
    public ResponseEntity<FileListResponse> listFiles(
        ...
        FileFilter finalFilter = FileFilter.byAuthor(currentUserId);
        ...
        List<FileMetadata> metadataList =
                fileStorageService.listFiles(currentTenantId, finalFilter).join();
